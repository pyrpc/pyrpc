...
Types are generated by the server-side `pyrpc dev` / `pyrpc watch` / `pyrpc codegen` commands into `<client>/__pyrpc.d.ts`, and resolved via a tsconfig path alias that `pyrpc dev` injects into your `tsconfig.json` (using jsonc-edit). No install-time steps required.
