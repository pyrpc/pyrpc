...
import httpx
import pytest
from pyrpc_server import rpc, asgi_app, RPCClient, default_registry
...
For TypeScript, you can wrap `pyRPCClient` to inject custom headers or use a different `fetch` implementation.
