...
Run `pyrpc dev`. On first run it walks you through setup, creates a `pyrpc.json` config, and generates types automatically. For CI, use `pyrpc codegen`.

## What is the distribution mode?

Workspace mode (default) writes types directly to your client project for monorepos. Server mode exposes types via `GET /rpc` for separate repositories.
