...
### vs gRPC
- **pyRPC**: Uses HTTP/JSON, no `.proto` files needed, developer-friendly
- **gRPC**: Requires Protocol Buffers, steeper learning curve, binary protocol

### vs FastAPI
- **pyRPC**: End-to-end type safety from server to client, automatic client generation
...
```python
# Example: pyRPC server
from pyrpc_server import Router
