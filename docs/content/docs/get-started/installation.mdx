...
On first run, `pyrpc dev` walks you through setup interactively - entry module, client project root, and frontend framework. It creates a `pyrpc.json` config file, generates types, and starts the development server with auto-regeneration on file changes.

You can also pass flags to skip the wizard:

```bash
pyrpc dev --yes                          # auto-detect everything
...
For multiple frontend projects, use `clients`:
