...
`pyrpc dev` serves the app with uvicorn (default `http://127.0.0.1:8000/rpc`), generates `__pyrpc.ts` at the client project root, and re-generates it on every `.py` save.
...
Note: the `baseUrl` must match the port your Flask app is served on. `pyrpc dev` uses port `8000` by default; if you run the app directly with `app.run(port=5000)` instead, use `5000`.
