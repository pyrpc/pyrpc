...
`pyrpc dev` starts uvicorn, generates `__pyrpc.ts` at the client project root, and re-generates it on every `.py` save. If a server is already running on that port, it skips starting uvicorn and only runs the type watcher.
