...
The client sends `{"jsonrpc": "2.0", "method": "add", "params": {"a": 1, "b": 2}, "id": 1}`. pyRPC deserializes params, calls `add(a=1, b=2)`, and returns the result in the JSON-RPC response.
...
Procedures can be sync or async. pyRPC detects awaitables and awaits them:
