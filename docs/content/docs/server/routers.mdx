...
pyRPC uses a **procedure registry** to hold your RPC procedures. You register procedures with `@rpc`, then mount the registry onto your app using an adapter. There is no separate router object — the registry is the router.
...
When a request arrives, pyRPC looks up the procedure by name in the registry and invokes it with the request params.
...
- **ASGI** — Use `PyRPCAsgiApp` or `asgi_app` for a standalone app
