...
export function resolveLinkItems({ links = [], githubUrl }: { links?: LinkItemType[]; githubUrl?: string }): LinkItemType[] {
  const result = [...links];
  if (githubUrl) {
    result.push({
      type: 'icon',
...
export function useLinkItems({ githubUrl, links }: { githubUrl?: string; links?: LinkItemType[] }) {
  return useMemo(() => {
