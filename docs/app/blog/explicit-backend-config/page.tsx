...
export default function ExplicitBackendConfigPost() {
 ...
 <p>
 v0.13.0&rsquo;s headline change looks bureaucratic, a JSON schema edit. It is actually a
 ...
 <li><strong>Invalid input fails before anything else:</strong> <code>--framework express</code> exits immediately with the four valid choices, before config loading, before module resolution.</li>
 </ul>
...
spec = parse_backend(cfg) # None for absent/invalid/legacy`}</pre>
 ...
 <code>parse_backend() is None</code>, no migration code paths, no version field to
 ...
 schema key. Explicitness is not ceremony here, it is what makes the other features
