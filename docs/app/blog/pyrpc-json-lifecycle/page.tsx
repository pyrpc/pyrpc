...
export default function PyrpcJsonLifecyclePost() {
    return (
        <article className="max-w-3xl mx-auto px-6 py-20">
            ...
            <section className="prose dark:prose-invert max-w-none text-sm leading-relaxed space-y-5 text-fd-muted-foreground [&_strong]:text-fd-foreground">
                <p>
                    Everything pyRPC needs to know about your project &mdash; which module to scan, which
                    frontends to feed &mdash; lives in one small JSON file. Tracing that file through the system is
    ...
    return path`}</pre>
                <p>
                    ...
                    &mdash; details that keep the file pleasant in pull requests. The result looks like:
                </p>
    ...
    try:
        ...
        return None`}</pre>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>dev</strong> &mdash; reads <code>module</code> and <code>client</code>/<code>clients</code>, imports the module, generates types, starts the server.</li>
                    <li><strong>watch</strong> &mdash; reads the same fields; the whole command is &ldquo;read config, then do the dev type-loop without a server.&rdquo;</li>
                    <li><strong>codegen</strong> &mdash; uses <code>--client</code> (or the config&rsquo;s clients when invoked through dev), so a CI regenerate honors the same layout.</li>
                    <li><strong>_get_clients</strong> &mdash; normalizes <code>client</code> or <code>clients</code> into one list, so downstream code never branches.</li>
                </ul>
...
# re-wire regen callback, restart uvicorn if module changed and owned, regen now`}</pre>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Repeatability</strong> &mdash; the same project behaves the same way across <code>dev</code>, <code>watch</code>, CI, and every teammate&rsquo;s machine.</li>
                    <li><strong>Reviewability</strong> &mdash; moving a client root is a one-line diff that shows up in a PR, not a secret flag you carry in a Makefile.</li>
                    <li><strong>Live behavior</strong> &mdash; a file can be watched; a command-line invocation cannot. The watcher turns config edits into running-system changes.</li>
                </ul>
                <p>
                    From a wizard prompt to a file being hot-reloaded by a running watcher, <code>pyrpc.json</code>
                    is the spine of the CLI. Every feature &mdash; multi-client, zero-codegen, non-interactive
                    setup &mdash; is really a rule about how this file is born, read, and kept in sync.
                </p>
                <p>
                    Read the full
