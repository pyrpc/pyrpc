...
export default function UvicornReloadDefaultPost() {
 ...
 you&rsquo;d recognize, and by default that command line ends with
 ...
 variable name, no <code>:other</code> guessing, because <code>_import_module</code>
 ...
 <p>
 <code>--log-level error</code> keeps the terminal quiet, the same reason the early
 ...
 <code>default_router.reload_module</code> purely to rebuild the TypeScript schema, 
 ...
 <code>True</code>, and that managed process is terminated on shutdown, Ctrl+C in
 ...
 actually gone before the port is reused, otherwise the restart would race
 ...
 uvicorn and calls <code>_start_uvicorn(new_module)</code>, but only when
 ...
 <ul>
 <li>You work in a large codebase and uvicorn&rsquo;s reloader, which stat-polls the files it watches, adds visible churn, or restarts the server for files it doesn&rsquo;t even import.</li>
 ...
 command, or a module change in <code>pyrpc.json</code>, still works, because
