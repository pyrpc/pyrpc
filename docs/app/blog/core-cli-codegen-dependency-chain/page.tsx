...
export default function CoreCliCodegenPost() {
 ...
 This was not an accident, it was the result of working through what each
 ...
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">Router</code>, the procedure registry.</li>
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">@rpc</code> / <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">@model</code>, decorators for registering procedures and models.</li>
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">handle_request</code>, the core RPC interpreter.</li>
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">PyRPCAsgiApp</code>, the ASGI transport layer.</li>
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">RPCClient</code>, the Python client for calling remote servers.</li>
 ...
 to install the CLI. But the dependency is purely structural, core never
 ...
 is a hard code dependency, the import happens at CLI startup.
 </p>
 <p>
 ...
 appropriate here because they are only needed when running the CLI, 
 ...
 <ul className="text-fd-muted-foreground">
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">jinja2</code>, for rendering TypeScript from templates.</li>
 <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">jsonschema-ts</code>, for converting Pydantic model JSON Schema to TypeScript interfaces.</li>
 </ul>
 <p>
...
{`pip install pyrpc-core
 ...
 console, the watcher, the shell, the serve command, all of it is
 ...
 pyrpc-codegen is a pure library, no side effects, no network calls,
 no <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">[project.scripts]</code>. pyrpc-cli is a tool, it orchestrates
 libraries and adds interactivity. pyrpc-core is an application library, 
...
pyrpc codegen https://api.example.com/rpc`}
 ...
 <ul className="text-fd-muted-foreground">
 <li><strong>First-run setup</strong>, <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc dev</code> with no config prompts for framework and entry point, saves to <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">[tool.pyrpc]</code>, installs adapter.</li>
 <li><strong>Adapter auto-install</strong>, <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">{'pip install pyrpc-core[{framework}]'}</code> based on framework choice.</li>
 <li><strong><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">--reconfigure</code></strong>, re-run setup prompts to switch frameworks or entry points.</li>
 <li><strong>Optional <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc shell</code> removal</strong>, the shell lives entirely in pyrpc-cli and can be deprecated without touching core or codegen.</li>
 </ul>
 <p>
