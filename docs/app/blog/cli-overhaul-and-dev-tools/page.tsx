        ...
        <article className="max-w-3xl mx-auto px-6 py-20">
            ...
            <section className="prose dark:prose-invert max-w-none text-sm leading-relaxed space-y-5 text-fd-muted-foreground [&_strong]:text-fd-foreground">
                ...
                <p>
                    ...
                    <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">undefined</code> is a terrible error signal &mdash; it produces a confusing
...
The .rpc prefix was removed for a cleaner API.`}
                ...
                <p>
                    This pattern &mdash; a proxy that throws helpful errors instead of silently
                    failing &mdash; is something we are applying across the client surface area.
                ...
                <p>
                    ...
                    (e.g. <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">User</code>) &mdash; no interface definition, no property types, no
                ...
                <ul className="text-fd-muted-foreground">
                    <li><strong>Ship a Python reimplementation</strong> of JSON Schema to TypeScript &mdash; months of work to handle allOf, anyOf, oneOf, $ref, $defs, enums, tuples, circular refs.</li>
                    <li><strong>Inline Jinja2 templates</strong> to walk schema properties &mdash; fragile, would need constant updates as JSON Schema evolves.</li>
                    <li><strong>Delegate to json-schema-to-typescript</strong> (npm, 3.3k stars, 705 dependents, used by Microsoft/Amazon/Expo/Webpack) &mdash; mature, tested, handles every edge case.</li>
                </ul>
                <p>
                    ...
                    zero-dependency Python stdlib &mdash; the only requirement is Node.js 18+
                    with npx available.
                </p>
                <p>
                ...
                <ul className="text-fd-muted-foreground">
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">collect_defs(*schemas)</code> &mdash; extracts and merges <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">$defs</code> from Pydantic JSON Schema output.</li>
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">convert_all(defs)</code> &mdash; converts all definitions to TypeScript interfaces in a single npx call.</li>
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">assemble(models, procedures)</code> &mdash; combines model and procedure types into the final output file.</li>
                </ul>
                <p>
...
export interface Types {
  createUser(user: User): Promise<User>;
                ...
                <p>
                    ...
                    file and generated TypeScript. The two-step workflow was confusing &mdash;
                ...
                <ul className="text-fd-muted-foreground">
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc codegen app.main</code> &mdash; import a Python module, extract schemas, generate TypeScript. One command end-to-end.</li>
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc codegen pyrpc-schema.json</code> &mdash; read a schema file, generate TypeScript. Same behavior as before.</li>
                    <li><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc codegen http://localhost:8000</code> &mdash; fetch schema from a running server, generate TypeScript. Same behavior as before.</li>
                </ul>
                <p>
                    ...
                    serialization &mdash; the serialization gap that made model interfaces
                    impossible.
                </p>
                ...
                <p>
                    ...
                    <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc</code> was not a real pip package &mdash; the ASGI app lived in
...
[5 procs] >>> exit`}
                ...
                <p>
                    The design was inspired by <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">docker exec</code> &mdash; the server keeps running
                ...
                <ul className="text-fd-muted-foreground">
                    <li><strong>tRPC</strong> has no CLI at all &mdash; types flow through <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">import type</code> at compile time. This works because tRPC is single-language (TypeScript). pyrpc bridges Python and TypeScript, which is a different problem.</li>
                    ...
                    <li><strong>Docker exec</strong> is a system-level shell inside a container, not an application REPL. But the UX pattern &mdash; server runs in background, exec into it for debugging &mdash; directly inspired <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc shell</code>.</li>
                </ul>
                <p>
                ...
                <p>
                    ...
                    start a server, and <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc</code> drops you into a shell automatically &mdash; no
                ...
                <ol className="text-fd-muted-foreground">
                    <li><strong>pyrpc init</strong> &mdash; framework-aware scaffolding that detects FastAPI, Flask, or raw pyRPC and generates project structure.</li>
                    <li><strong>pyrpc shell as default</strong> &mdash; running <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc</code> with no args opens the shell if a server is detected.</li>
