        ...
        <article className="max-w-3xl mx-auto px-6 py-20">
            ...
            <section className="prose dark:prose-invert max-w-none text-sm leading-relaxed space-y-5 text-fd-muted-foreground [&_strong]:text-fd-foreground">
                <p>
                    ...
                    pyRPC&rsquo;s first-run wizard asks the minimum that cannot be guessed &mdash; and guesses
                ...
                <p>
                    ...
                    &mdash; <code>main.py</code>, <code>server.py</code>, <code>app.py</code>, <code>app/main.py</code>
                    &mdash; and pre-fills the first hit:
                </p>
...
module = questionary.text(
    ...
    instruction="(e.g. main, app.server, the file that calls mount_fastapi/mount_flask)",
                ...
                <p>
                    ...
                    interesting part &mdash; the wizard branches on evidence before it asks:
                </p>
...
# multiple: list them, then select or enter manually`}</pre>
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Nothing detected</strong> &mdash; asks for a client root (default <code>.</code>) and a framework from the menu (<code>Next.js, Nuxt, Svelte, Vite, Astro, Other</code>).</li>
                    <li><strong>Exactly one project detected</strong> &mdash; both fields come pre-filled from the detection; you confirm and move on.</li>
                    <li><strong>Several projects detected</strong> &mdash; prints the list, then lets you multi-select the ones to wire up, writing <code>clients</code> and <code>framework: "Mixed"</code>.</li>
                </ul>
                <p>
    ...
    with open(cfg_path) as f:
        cfg = json.load(f)`}</pre>
                <p>
                    ...
                    to run again &mdash; via <code>--reconfigure</code> &mdash; it always writes a fresh config
                ...
                <ul className="list-disc pl-5 space-y-2">
                    ...
                    <li><strong>Write once, read forever.</strong> The config is durable, committed, and re-read by <code>dev</code>, <code>watch</code>, and <code>codegen</code> &mdash; the wizard is a one-time cost.</li>
                    <li><strong>Graceful exits.</strong> <code>None</code> from any prompt (Ctrl+C) raises <code>typer.Exit(code=0)</code> &mdash; canceling setup is a clean no-op, not an error.</li>
                </ul>
                <p>
