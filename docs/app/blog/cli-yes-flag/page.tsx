...
export default function CliYesFlagPost() {
        ...
        <article className="max-w-3xl mx-auto px-6 py-20">
            ...
            <section className="prose dark:prose-invert max-w-none text-sm leading-relaxed space-y-5 text-fd-muted-foreground [&_strong]:text-fd-foreground">
                <p>
                    ...
                    build, or a setup script cannot answer prompts &mdash; and a tool that blocks on input in one
    ...
    if cfg_path is None:
        ...
        console.print(f"  [green]✓[/green]  pyrpc.json created")`}</pre>
                <p>
                    ...
                    <code>pyrpc.json</code> only if one does not already exist &mdash; a pre-existing config is
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Module</strong> &mdash; first match of <code>main.py</code>, <code>server.py</code>, <code>app.py</code>, <code>app/main.py</code>.</li>
                    <li><strong>Client + framework</strong> &mdash; via <code>_find_frontend_projects</code>. Exactly one project &rarr; it is used. Several projects &rarr; hard error listing them and pointing at <code>--client</code>. None &rarr; no client configured, framework <code>"Other"</code>.</li>
                </ul>
                <p>
                    The multiple-projects case deserves emphasis. Rather than silently pick the first frontend it
                    finds &mdash; a guess that would generate types into the wrong app &mdash; the CLI refuses and
    ...
    raise typer.Exit(1)`}</pre>
                <p>
                    Non-interactive mode still refuses to guess in ambiguous situations &mdash; it just reports the
    ...
    console.print(f"  [dim]module={cfg['module']}  (no client configured)[/dim]")`}</pre>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>CI</strong> &mdash; regenerate types in a build step with <code>--yes --module main --client ./frontend</code>, deterministic every run.</li>
                    <li><strong>Container image builds</strong> &mdash; a Dockerfile layer that pre-generates types cannot hang on a prompt; <code>--yes</code> guarantees it cannot.</li>
                    <li><strong>Scripted onboarding</strong> &mdash; repo setup scripts, devcontainer post-create hooks, and Makefile targets can configure pyRPC without user input.</li>
                </ul>
                <p>
