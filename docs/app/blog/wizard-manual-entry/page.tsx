...
export default function WizardManualEntryPost() {
 ...
 same checkbox list as the detected projects, a design that
 ...
 <code>(path, framework)</code> pairs, for example
 ...
 framework select is pre-set to the detected framework, the
 wizard does the work for you. The interesting case, and the
 one v0.11.1 fixes, is when <em>multiple</em> projects are
...
while True:
 ...
 <p>
 The checkbox choices are display strings, each detected
...
return {"module": module, "framework": "Mixed", "clients": clients}`}</pre>
 ...
 that matters, Next.js, Vite, and so on, lives on
 ...
 told it, the exact kind of quiet data loss that setup flows
 must never have.
 </p>
 <p>
 ...
 mutually exclusive choice between modes, &ldquo;how should I
 configure clients?&rdquo; A <code>questionary.checkbox</code> is a
 multi-select of values <em>within</em> a single mode, 
 ...
 actually a redirect, and that checking it erased the other
 selections, is exactly the kind of surprise that erodes
 ...
 feature and selects its sub-options, all share the same
 ...
 confirm with nothing checked, and all options start
...
, skipping type generation</code>: no error, no retry, just a
 ...
 the yellow hint <code>No projects selected, choose at least
 ...
 --yes</code> or a CI job must never block on a loop, it
 ...
 everywhere, never a crash, never a swallowed
 ...
 runs exactly once, the resulting <code>pyrpc.json</code>
