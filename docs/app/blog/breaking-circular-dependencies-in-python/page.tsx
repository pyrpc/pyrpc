...
def _cmd_pull(module_path):
 ...
 terrible, the package installs successfully, but most commands
...
pyrpc-codegen`}
 ...
 function, it calls <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">get_registry_schema(default_router)</code>,
...
{`Before:
 ...
 pyrpc-codegen ΓåÆ pyrpc-core (cycle when pyrpc-core ΓåÉ pyrpc-codegen)



After:

 pyrpc-core ΓåÆ pyrpc-cli ΓåÆ pyrpc-codegen
 ...
 layer, it is developer tooling that orchestrates the runtime and
...
def serve(module_path: str, host: str, port: int):
 ...
 <li><strong>Merge</strong>, when the packages are never used independently and have similar dependency profiles. Example: a library and its type stubs.</li>

 <li><strong>Remove backward dep</strong>, when the backward dependency is a single function call and the package is meaningful without it. Example: a CLI tool that has a "use local" vs "use remote" mode.</li>

 <li><strong>Invert + shared utils</strong>, when the overlapping code is a clean, extractable concern. Example: shared validation logic between a server and a client package.</li>

 <li><strong>Extract intermediary</strong>, when the overlapping concern is a distinct architectural layer. Example: a CLI that orchestrates a runtime and a code generator.</li>
