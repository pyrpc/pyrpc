...
export default function CodegenRefactorPost() {
 ...
 series of improvements that reshape how you interact with pyRPC, especially
 ...
 <li><strong><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc pull</code></strong>, import a Python module, walk its <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">get_registry_schema()</code>, and save a portable JSON schema file. This is a server-side or CI tool. It pulls the schema <em>out</em> of Python and puts it on disk so codegen can run anywhere.</li>
 <li><strong><code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc codegen</code></strong>, read a schema file or a URL, generate TypeScript types. That is it.</li>
 </ul>
 <p>
 This mirrors Prisma&rsquo;s <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">db pull</code> / <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">generate</code> split, one command to extract
 ...
 schema over HTTP, transforms it, and writes TypeScript, all without ever
 ...
 with <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">PYRPC_URL</code> set, no Python backend setup required.
 </p>
 <p>
 ...
 the frontend-facing documentation, the quickstart, the TypeScript client
 ...
 for Python&rdquo; which implied it was a Python-only concept. The real
 ...
 backend is Python and their frontend is TypeScript, which describes a
 ...
 <ul className="text-fd-muted-foreground">
 <li><strong>Reporting</strong>, GitHub private advisories, what to include, expected response time, credit policy.</li>
 <li><strong>Supported versions</strong>, only the latest minor before 1.0.0, no SLA for pre-releases.</li>
 <li><strong>What is in scope</strong>, core protocol, adapter boundary, generated code, introspection endpoint, error responses.</li>
 <li><strong>What is not in scope</strong>, upstream dependencies, already-compromised machines, old versions.</li>
 <li><strong>What we do</strong> and <strong>what we cannot promise</strong>, transparent about the trust boundaries.</li>
 </ul>
 <p>
 ...
 procedure author&rsquo;s responsibility. This is an honest boundary, we
 ...
 <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">any</code> is a type that pyRPC cannot parse at all, which is increasingly
 ...
 <li><strong>React integration patterns</strong>, a docs page showing <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">createClient</code> with <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">useQuery</code>/<code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">useMutation</code>, auth headers, and error handling.</li>
 <li><strong>Pydantic model-to-interface generation</strong>, full TypeScript interfaces from Python model definitions.</li>
 <li><strong>Compatibility doc</strong>, what version numbers mean, safe vs breaking changes, rolling upgrade patterns.</li>
 </ol>
 <p>
 The CLI, the frontend DX, the security policy, and the documentation are in
 a good state. The remaining work is about depth, richer type generation,
