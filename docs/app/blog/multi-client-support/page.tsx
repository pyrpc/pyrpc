...
export default function MultiClientSupportPost() {
        ...
        <article className="max-w-3xl mx-auto px-6 py-20">
            ...
            <section className="prose dark:prose-invert max-w-none text-sm leading-relaxed space-y-5 text-fd-muted-foreground [&_strong]:text-fd-foreground">
                <p>
                    ...
                    grew a second consumer &mdash; a marketing site, an admin panel, a dashboard
                ...
                <p>
                    ...
                    &mdash; <code>dev</code>, <code>watch</code>, <code>codegen</code>, and the watcher&rsquo;s
                    regen callback &mdash; regenerates types for all of them.
                </p>
  ...
  "clients": ["./frontend", "./admin"]
                ...
                <p>
                    ...
                    configures several clients at once &mdash; the value is informational, a record of
    ...
    return n`}</pre>
                <p>
                    ...
                    never stored &mdash; there is exactly one place that knows the convention
                ...
                <p>
                    Three different entry points needed to do the same thing &mdash; generate types for all
                    clients on startup or change &mdash; and they all now call <code>_regenerate_clients</code>:
                </p>
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>dev</strong> &mdash; after importing the module on startup, and again whenever the debounced regen callback fires on a <code>.py</code> change.</li>
                    <li><strong>watch</strong> &mdash; once at startup, then through the same regen callback for the lifetime of the process.</li>
                    <li><strong>the regen callback</strong> &mdash; <code>_do_regen</code>, shared by both of the above.</li>
                </ul>
                <p>
                    The startup messages differ slightly by count &mdash; <code>types generated (2 procs) &rarr;
                    ./frontend</code> for one client versus <code>types generated (2 procs) for 2 clients</code>
                    for several &mdash; but the work is identical, so behavior can&rsquo;t drift between commands.
                </p>
...
if output_changed:
    ...
    console.print(f"  [dim]clients → {client_dirs}[/dim]")`}</pre>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Separate teams, one API</strong> &mdash; a backend team ships the same typed contract to a web app and an admin tool without maintaining two configs or two servers.</li>
                    <li><strong>Monorepo symmetry</strong> &mdash; every frontend in <code>packages/*</code> or <code>apps/*</code> gets its own committed <code>__pyrpc.d.ts</code>, tracked in git like any other source file.</li>
                    <li><strong>Consistent CI</strong> &mdash; <code>codegen --client</code> and <code>watch --client</code> normalize to the same path convention, so a CI job can regenerate a single client without touching the others.</li>
                </ul>
                <p>
                    ...
                    framework lives there &mdash; that was only ever needed to pick an output path, and the
