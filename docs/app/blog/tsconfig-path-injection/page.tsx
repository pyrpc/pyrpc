...
export default function TsconfigPathInjectionPost() {
 ...
 make that import resolve to your file instead of the published package, and in
 ...
 <p>
 The naive approach, <code>json.load</code> the tsconfig, mutate the dict, write it
 back with <code>json.dump</code>, destroys anything that is not strict JSON. Real
 <code>tsconfig.json</code> files routinely contain:
 </p>
 <ul className="list-disc pl-5 space-y-2">
 <li><strong>Comments</strong>, <code>// ...</code> and <code>/* ... */</code> are legal JSONC but illegal JSON, and teams use them to explain <code>paths</code> entries and <code>baseUrl</code> decisions.</li>
 <li><strong>Trailing commas</strong>, every frontend toolchain happily accepts them; a round-trip through <code>json.dump</code> silently deletes them, producing noisy one-line diffs for unrelated fields.</li>
 </ul>
 <p>
 ...
 write of a value that could never collide with real config, the string
 <code>"SENTINEL"</code>, and inspects the edit it would make:
 </p>
 ...
 path already exists and that edit is a <em>replacement</em> of the existing value, the
 ...
 returns <code>None</code>. It&rsquo;s a read implemented with a write function, a neat
 ...
 exactly <code>["./__pyrpc.d.ts"]</code>, there is nothing to do, a rerun is a no-op.
...
if existing is not None:
 ...
 <p>
 If the alias points somewhere else, say a developer previously wired
 <code>"./src/__pyrpc.d.ts"</code> by hand, pyRPC does <strong>not</strong> silently
 ...
 <ul className="list-disc pl-5 space-y-2">
 <li><strong>Missing <code>compilerOptions</code></strong>, the edit creates the whole branch and preserves an existing comment.</li>
 <li><strong>Missing <code>paths</code></strong>, injects the mapping into existing <code>compilerOptions</code> without disturbing <code>strict</code> or <code>include</code>.</li>
 <li><strong>Existing paths with comments and trailing commas</strong>, keeps <code>"~/*"</code>, the <code>// some comment</code>, and <code>/* trailing comma above! */</code> all intact.</li>
 <li><strong>Already-correct alias</strong>, the file is returned byte-for-byte unchanged (idempotency).</li>
 <li><strong>Conflicting alias</strong>, raises <code>RuntimeError</code> instead of overwriting.</li>
 <li><strong>No <code>tsconfig.json</code></strong>, returns <code>True</code> and creates nothing; the file may not exist yet.</li>
 </ul>
 <p>
...
mdash; rather than a documented manual step, means the source-tree types feature is zero-config by
