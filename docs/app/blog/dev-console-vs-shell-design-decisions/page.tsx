...
"Hello World"`}
                ...
                <ul className="text-fd-muted-foreground">
                    <li><strong>No HTTP call overhead</strong> &mdash; calling a procedure through the console is a direct Python function call, not a round-trip through the network stack.</li>
                    <li><strong>Works even if the server fails to start</strong> &mdash; the registry was populated during the import phase, which happens before the server starts. If uvicorn crashes, the console still works.</li>
                    <li><strong>No serialization-deserialization round-trip</strong> &mdash; arguments are passed as Python objects, not JSON strings that get parsed again.</li>
                    <li><strong>No HTTP stack dependency</strong> &mdash; the console does not need to import httpx or any HTTP client.</li>
                </ul>
                <p>
                    ...
                    command. There is no HTTP endpoint to call &mdash; it calls the Python
...
└──────────────────────────────────────────┘`}
                ...
                <ol className="text-fd-muted-foreground">
                    <li>
                        ...
                        console is useless &mdash; you have lost your debugging surface because
                        the server crashed.
                    </li>
                    <li>
...
class _ShellConsole:
    def _schemas(self) -> dict:
        ...
        return resp.json()`}
                ...
                <ul className="text-fd-muted-foreground">
                    <li><strong>A dedicated <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyrpc.json</code></strong> &mdash; clean separation, follows the ESLint/Prettier pattern.</li>
                    <li><strong>A <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">pyproject.toml</code> <code className="text-[10px] font-mono bg-fd-muted px-1.5 py-0.5 rounded">[tool.pyrpc]</code> section</strong> &mdash; follows the Black/ruff/FastAPI pattern, no extra files.</li>
                    <li><strong>Environment variables only</strong> &mdash; minimal, but poor discoverability.</li>
                </ul>
                <p>
...
Installing pyrpc-fastapi adapter...
Starting dev server...`}
                </pre>
                <p>
                ...
                <p>
                    ...
                    a hint to uninstall it manually &mdash; we do not auto-uninstall because
                ...
                <ul className="text-fd-muted-foreground">
                    <li><strong>Zero-config on the happy path</strong> &mdash; answer three prompts, done.</li>
                    <li><strong>Explicit and predictable</strong> &mdash; no auto-detection magic, no guessing.</li>
                    <li><strong>Framework-agnostic</strong> &mdash; the same CLI commands work for FastAPI, Flask, or raw ASGI.</li>
                    <li><strong>Config-as-code</strong> &mdash; pyproject.toml is checked into version control, so the whole team gets the same setup.</li>
                </ul>
                <p>
