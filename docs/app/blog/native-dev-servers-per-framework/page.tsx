...
export default function NativeDevServersPost() {
 ...
 shrugged, that is their normal. Flask users got an ASGI-wrapper detour. Django users were
 ...
 <li><strong>Django is not just WSGI/ASGI.</strong> Settings loading, app registry initialization, and management commands are framework machinery. Running Django any other way re-implements pieces of <code>runserver</code> badly, static files handling and autoreload semantics being the classic victims.</li>
 <li><strong>Flask&rsquo;s dev server already works.</strong> Wrapping a WSGI app in an ASGI bridge to serve it under uvicorn adds a dependency, a thread hop, and a second stack trace format to debug, to arrive at the same behavior <code>flask run</code> gives you out of the box.</li>
 ...
 <p>
 The resolver does not spawn anything. It returns a frozen <code>LaunchPlan</code>, argv,
 optional env additions, optional working directory, and the caller spawns once:
 </p>
...
django -> python <path>/manage.py runserver H:P [--noreload] (cwd = manage.py dir)`}</pre>
 ...
 sets <code>cwd</code> accordingly, because <code>manage.py runserver</code> only works
