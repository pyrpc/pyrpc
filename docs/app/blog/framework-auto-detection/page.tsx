...
export default function FrameworkAutoDetectionPost() {
    ...
    ("astro.config.mjs",  "Astro"),
                ...
                <p>
                    Every framework above has exactly one unavoidable config file &mdash; the file its own tooling
    ...
    return None`}</pre>
                <p>
                    ...
                    also does no content parsing &mdash; existence is the signal, which keeps the check cheap and
    ...
    return projects`}</pre>
                <p>
                    ...
                    <code>os.walk</code> uses to plan its descent &mdash; pruned directories are never
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>The wizard</strong> &mdash; pre-fills client root and framework when exactly one project is found, or offers multi-select when several are.</li>
                    <li><strong><code>dev --yes</code></strong> &mdash; with exactly one detected project, config is written with no prompts at all.</li>
                    <li><strong>Error quality</strong> &mdash; with several projects and no explicit <code>--client</code>, <code>dev --yes</code> refuses to guess and lists the candidates with the exact flag to disambiguate.</li>
                </ul>
                <p>
                    The framework label ends up as a single informational field in <code>pyrpc.json</code>. It is
                    a record of the setup decision, not a runtime dependency &mdash; the generated types and the
