...
export default function DebouncedRegenThreadsPost() {
 ...
 <p>
 Type regeneration looks simple from the outside, a file changes, types update. Under
...
def _schedule_regen():
 ...
 timer fires 300ms later and regeneration runs against the final, complete file, never
 ...
 <p>
 <code>schedule_regen</code> is called from two threads, the file watcher and the
 ...
 timer callback itself can be slow, a regeneration imports the module and reads the whole
...
, the running regen reads the same latest module state, so dropping the duplicate
 changes nothing.
 </p>
 ...
 scoping rules turned a missing import into a silent <code>NameError</code> at runtime, 
 ...
 imports), silent failure beats no failure only by being detectable.
 </p>
 ...
 <li><strong>Watch thread</strong>, produces events, filtered to <code>.py</code> and config changes.</li>
 <li><strong>Timer (debounced)</strong>, collapses bursts, guards against half-written files.</li>
 <li><strong>Timer lock</strong>, serializes scheduling across threads.</li>
 <li><strong>Regen lock</strong>, prevents concurrent writes to the same output file.</li>
 <li><strong>The log line</strong>, the only outward signal the loop is alive.</li>
 </ul>
 <p>
