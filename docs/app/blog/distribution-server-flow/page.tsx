...
export default function ServerFlowPost() {
 ...
 smaller because the server&rsquo;s responsibility is narrower, serve the
 ...
 <p>
 In server mode, pyrpc still reloads the router and extracts the schema, 
...
{" Run npx pyrpc sync on the client to regenerate types"}</pre>
 ...
 <p>
 It would be simpler to always write types, even in server mode, just omit
 ...
 client decides when to fetch new types, on deploy, on demand, on a
