...
export default function ModuleReloadWatcherPost() {
 ...
 regenerated types on every save, but the types were sometimes stale, reflecting the
 ...
 <code>pyrpc codegen</code> that is exactly right, but the watcher runs inside a
...
schemas = get_registry_schema(default_router)`}</pre>
 ...
 regenerate, identical to the last time.
 </p>
 ...
 <li><strong>Atomic swap</strong>, old procedures are snapshotted and the router is cleared before <code>importlib.reload</code> re-runs the module, which re-fires every <code>@rpc</code> decorator and repopulates the router.</li>
 <li><strong>Rollback on failure</strong>, if the module raises during reload (a syntax error, a missing import, a runtime error at module scope), the old procedures are restored and the exception propagates. A broken save never leaves you with an empty router.</li>
 <li><strong>Empty-guard</strong>, if the reloaded module exports no procedures (you deleted the last <code>@rpc</code>, or the module&rsquo;s decorator is bound to a different router), the old set is restored and <code>False</code> is returned.</li>
 </ul>
 <p>
 ...
 <p>
 <code>reload=False</code>, used by <code>dev</code> and <code>watch</code> at
 startup, and by one-shot <code>codegen</code>, does a normal import: the process is
 fresh, nothing is cached, an import is correct. <code>reload=True</code>, used by the
 debounced regen callback after a save, goes through <code>reload_module</code>, with
 ...
 is a one-word fix with a big contract, <em>save, wait 300ms, types reflect exactly
 what you wrote</em>, and it restores the core promise of the zero-codegen workflow.
 </p>
 <p>
 Read the full
